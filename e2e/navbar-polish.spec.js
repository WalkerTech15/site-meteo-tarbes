 ...
 * duplicate.
 *
 * The Simple/Détaillé display-mode toggle renders TWICE and stays in sync:
 * once in this navbar (#modeToggle, desktop-width only — see the 901/900px
 * breakpoint tests below), and once in the sidebar (#modeToggleSide, always
 * visible there, both as the permanent desktop rail and the mobile drawer).
 * The selection/keyboard/i18n coverage below exercises the navbar copy,
 * since that's this file's own scope; #modeToggleSide's equivalent behavior
 * is covered in app.spec.js (tests 5, 62-64). */
...
test.describe("navbar polish: the 901/900px drawer breakpoint", () => {
  ...
  test("900px switches to the drawer layout — hamburger visible, mode toggle hidden in the navbar", async ({
    page,
  }) => {
    await freshAt(page, { width: 900, height: 800 });
    await expect(page.locator("#burgerBtn")).toBeVisible();
    await expect(page.locator("#modeToggle")).toBeHidden(); /* still present, just css-hidden */
    const display = await page
      .locator(".topnav-inner")
      .evaluate((el) => getComputedStyle(el).display);
    ...
    /* the sidebar's own copy is the reachable one at this width */
...
test.describe("navbar polish: Simple/Détaillé selection is clear without a heavy shadow", () => {
  test("the selected option carries a distinct background (thumb), border and heavier weight", async ({
    ...
    /* font-weight transitions over 0.25s (toggles.css) — poll instead of a
       single read, so this doesn't race the transition's in-flight value. */
    await expect
      .poll(async () => {
        const detailedWeightAfter = await detailed.evaluate((el) =>
          Number(getComputedStyle(el).fontWeight),
        );
        const simpleWeightAfter = await simple.evaluate((el) =>
          Number(getComputedStyle(el).fontWeight),
        );
        return detailedWeightAfter > simpleWeightAfter;
      })
      .toBe(true);
  });
...
test.describe("navbar polish: keyboard focus is visible", () => {
  test("Tabbing to the search field, mode toggle and theme button shows a focus outline", async ({
    ...
    for (const sel of [
      "#modeToggle button[data-mode='detailed']",
      "#modeToggleSide button[data-mode='detailed']",
      "#themeBtn",
      "#langBtn",
    ]) {
...
test.describe("navbar polish: bilingual accessible labels", () => {
  test("French (default)", async ({ page }) => {
    ...
    for (const id of ["#modeToggle", "#modeToggleSide"]) {
      await expect(page.locator(`${id} button[data-mode="simple"]`)).toHaveText("Simple");
      await expect(page.locator(`${id} button[data-mode="detailed"]`)).toHaveText("Détaillé");
    }
  });
  ...
  test("English", async ({ page }) => {
    ...
    for (const id of ["#modeToggle", "#modeToggleSide"]) {
      await expect(page.locator(`${id} button[data-mode="simple"]`)).toHaveText("Simple");
      await expect(page.locator(`${id} button[data-mode="detailed"]`)).toHaveText("Detailed");
    }
  });
});
...
/* Two copies of the same Simple/Détaillé control — #modeToggle in this
 * navbar, #modeToggleSide in the sidebar — that must always agree. Both
 * exist in the DOM at every width; only visibility changes at the drawer
 * breakpoint (#modeToggle css-hidden below it, #modeToggleSide always
 * shown). Changing either one has to update BOTH controls and the actual
 * body[data-mode] immediately — see features/settings.js's setMode, which
 * syncs #modeToggle and #modeToggleSide from the same call. */
test.describe("navbar polish: #modeToggle and #modeToggleSide stay in sync", () => {
  test("both exist at every width; only the navbar copy's visibility changes at the drawer breakpoint", async ({
    page,
  }) => {
    await freshAt(page, { width: 1280, height: 800 });
    await expect(page.locator("#modeToggle")).toHaveCount(1);
  ...
  test("clicking the navbar toggle updates the sidebar toggle and the mode immediately", async ({
    page,
  }) => {
    await freshAt(page, { width: 1280, height: 800 });
    ...
    await expect(page.locator("body")).toHaveAttribute("data-mode", "detailed");
    await expect(page.locator('#modeToggle button[data-mode="detailed"]')).toHaveAttribute(
      "aria-checked",
      "true",
    );
    ...
    await expect(page.locator('#modeToggleSide button[data-mode="simple"]')).toHaveAttribute(
      "aria-checked",
      "false",
    );
  });

  test("clicking the sidebar toggle updates the navbar toggle and the mode immediately", async ({
    page,
  }) => {
    await freshAt(page, { width: 1280, height: 800 });
    await page.locator('#modeToggleSide button[data-mode="detailed"]').click();
    ...
    await expect(page.locator('#modeToggle button[data-mode="simple"]')).toHaveAttribute(
      "aria-checked",
      "false",
    );
  });

  test("the mode chosen via the drawer at mobile width is reflected on the navbar toggle back at desktop width", async ({
    page,
  }) => {
    await freshAt(page, { width: 1280, height: 800 });
    await expect(page.locator("#modeToggle")).toBeVisible();
...
/* The navbar with the mode toggle restored: logo, search, mode toggle, theme
 * and language all present, visible, balanced, and independently usable. */
test.describe("navbar polish: all navbar controls stay accessible and usable", () => {
  test("logo, search, mode toggle, theme and language are all present, and the actions group holds exactly three controls", async ({
    page,
  }) => {
    await freshAt(page, { width: 1280, height: 800 });
    ...
    /* the actions group is mode toggle + theme + language */
    const actionGroups = await page.locator(".topnav-actions > *").evaluateAll((els) => els.length);
    expect(actionGroups).toBe(3);
  });

  test("mode toggle, theme and language buttons are reachable by keyboard and operable in sequence", async ({
    page,
  }) => {
    await freshAt(page, { width: 1280, height: 800 });
    await page.locator("#searchInput").focus();
    ...
    await page.keyboard.press("Tab");
    await expect(page.locator('#modeToggle button[data-mode="detailed"]')).toBeFocused();
    await page.keyboard.press("Enter");
    await expect(page.locator("body")).toHaveAttribute("data-mode", "detailed");
